...
**When in doubt, the PRD and Technical-Spec win.** Stories here are *acceptance criteria*, not new requirements. If a story conflicts with the spec, the spec is correct and the story is stale — fix the story.
...
- **When** another device (or a forged cookie) hits `/api/r/<slug>/parties/<id>/stream` whose `party_session` cookie does not match `party.sessionToken`, *or* whose URL `<slug>` does not resolve to the party's `tenant_id`
...
- **When** the *other* device (not the one that seated) taps Undo within 60 seconds
