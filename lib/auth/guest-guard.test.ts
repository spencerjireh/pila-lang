...
vi.mock("@/lib/tenants/display-token", () => ({
  loadTenantBySlug: vi.fn(),
}));

vi.mock("@/lib/parties/lookup", () => ({
  findPartyById: vi.fn(),
...
async function loadMocks() {
  ...
  const tenants = await import("@/lib/tenants/display-token");
  const lookup = await import("@/lib/parties/lookup");
