...
export const DEMO_HISTORICAL = [
  { name: "Vikram Singh", partySize: 3, daysAgo: 14, waitMinutes: 5, phone: null },
  { name: "Meera Devi", partySize: 2, daysAgo: 13, waitMinutes: 10, phone: null },
  { name: "Arjun Kumar", partySize: 5, daysAgo: 12, waitMinutes: 20, phone: null },
  { name: "Sana Khan", partySize: 1, daysAgo: 10, waitMinutes: 15, phone: "+14155550101" },
  { name: "Rohan Gupta", partySize: 4, daysAgo: 8, waitMinutes: 30, phone: null },
  { name: "Zara Ahmed", partySize: 2, daysAgo: 6, waitMinutes: 8, phone: "+14155550102" },
  { name: "Aditya Nair", partySize: 6, daysAgo: 4, waitMinutes: 40, phone: null },
  { name: "Pooja Verma", partySize: 2, daysAgo: 3, waitMinutes: 12, phone: "+14155550103" },
  { name: "Deepak Reddy", partySize: 3, daysAgo: 2, waitMinutes: 18, phone: null },
  { name: "Ayesha Malik", partySize: 4, daysAgo: 1, waitMinutes: 25, phone: null },
] as const;
...
export async function resetDemoFixture(tenantId: string, now: Date = new Date()): Promise<ResetDemoResult> {
  return getDb().transaction(async (tx) => {
    ...
    if (partyIds.length > 0) {
      await tx.delete(notifications).where(inArray(notifications.partyId, partyIds.map((p) => p.id)));
    }
