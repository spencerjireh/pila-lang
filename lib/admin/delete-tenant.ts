...
export async function hardDeleteTenant(tenantId: string): Promise<HardDeleteResult> {
  return getDb().transaction(async (tx) => {
    ...
    await tx.update(tenants).set({ isOpen: false }).where(eq(tenants.id, tenantId));
    ...
    return { ok: true, slug: tenant.slug, affectedPartyIds: affected.map((a) => a.id) } as const;
  });
}
