...
test.describe("host queue", () => {
  test("snapshot shows waiting parties and new joins appear within 1s", async ({
    ...
    await expect(page.getByText("Fresh Charlie")).toBeVisible({ timeout: 5_000 });
  });
  ...
  test("seat removes from waiting and moves row to recently resolved", async ({
    ...
    await expect(page.getByRole("heading", { name: /waiting \(0\)/i })).toBeVisible({
      timeout: 15_000,
    });
    // Waiting empty message appears; party is no longer in any waiting row.
    await expect(page.getByText(/no one waiting right now/i)).toBeVisible({ timeout: 15_000 });
  });
  ...
  test("close-queue dialog flips pill and display reflects within 60s", async ({
    ...
    const { slug, password } = await tenantFactory({ name: "Close Test", isOpen: true });
    ...
    await expect(page.getByRole("button", { name: /open queue/i })).toBeVisible();
    await expect(
