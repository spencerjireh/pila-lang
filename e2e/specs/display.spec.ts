...
test.describe("display page", () => {
  test("renders tenant name and QR on open tenant with no flash", async ({ page, tenantFactory }) => {
    const { slug } = await tenantFactory({ name: "Display Test", isOpen: true });
  ...
  test("shows closed banner when tenant is closed", async ({ page, tenantFactory }) => {
  ...
  test("swaps QR for closed banner within 60s when host closes queue", async ({
    ...
    const { slug, password } = await tenantFactory({ name: "Live Close Test", isOpen: true });

    await page.goto(`/display/${slug}`);
    await expect(page.getByLabel(/QR code to join/i)).toBeVisible();

    const cookie = await hostLoginViaApi(request, slug, password);
    const res = await request.post(`/api/host/${slug}/close`, { headers: { cookie } });
