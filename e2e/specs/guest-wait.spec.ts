...
test.describe("guest wait page", () => {
  test("renders initial position from snapshot", async ({ page, tenantFactory, request }) => {
    ...
    const me = await joinAsGuest(page.request, slug, { name: "Me", partySize: 3 });
  ...
  test("position decreases within 1 second when party ahead is seated", async ({
    ...
    const ahead = await joinAsGuest(request, slug, { name: "Ahead", partySize: 2 });
    const me = await joinAsGuest(page.request, slug, { name: "Me", partySize: 3 });

    const hostCookie = await hostLoginViaApi(request, slug, password);
    await page.goto(me.waitUrl);

    const positionLocator = page.locator("[aria-live='polite'][aria-atomic='true']");
  ...
  test("transitions to seated terminal when seated", async ({
    ...
    const me = await joinAsGuest(page.request, slug, { name: "Me", partySize: 2 });
    ...
    await expect(page.getByRole("heading", { name: /your table is ready/i })).toBeVisible({
      timeout: 5000,
    });
  });

  test("leave flow transitions to left terminal", async ({ page, tenantFactory }) => {
    const { slug } = await tenantFactory({ name: "Leave Test" });
    const me = await joinAsGuest(page.request, slug, { name: "Goodbye", partySize: 1 });

    await page.goto(me.waitUrl);
    await page.getByRole("button", { name: /leave queue/i }).click();
    await page.getByRole("button", { name: /yes, leave/i }).click();

    await expect(page.getByRole("heading", { name: /you've left the queue/i })).toBeVisible();
  });
});
